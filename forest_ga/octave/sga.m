...
% Population
population = zeros(POPULATION_LIMIT,3,NUMBER_GENES);
next_population = zeros(POPULATION_LIMIT,3,NUMBER_GENES);
...
for trial=1:1:TRIAL_LIMIT
	...
	for iter=1:1:POPULATION_LIMIT
		accum = 0;
		potential = populator(NUMBER_GENES);
		population(iter,1,:) = fixer(potential,ADJ_MSK);
		accum += population(iter,1,:);

		potential = populator(NUMBER_GENES);
		for jter=1:1:NUMBER_GENES
			if potential(jter)>0 && accum(jter)>0
				potential(jter)=0;
			end
		end
		population(iter,2,:) = fixer(potential,ADJ_MSK);
		accum += population(iter,2,:);
		...
		population(iter,3,:) = fixer(potential,ADJ_MSK);
		accum += population(iter,3,:);
	end
%	population
...
%		clear sorted_fitnesses sorted_indices;
		...
		for individual=1:1:POPULATION_LIMIT
			sin_fit(individual) = fitness(population(individual,:,:),YIELD);
		end
...
%		end
		...
		First_Pop(1,trial,generation) = population(uni_ind(nc),1,:);
		First_Pop(1,trial,generation) += 2.*population(uni_ind(nc),2,:);
		First_Pop(1,trial,generation) += 3.*population(uni_ind(nc),3,:);
		if (nc>1)
			Second_Fit(1,trial,generation) = uni_fit(nc-1);
			Second_Pop(1,trial,generation) = population(uni_ind(nc-1),1,:);
			Second_Pop(1,trial,generation) += 2.*population(uni_ind(nc-1),2,:);
			Second_Pop(1,trial,generation) += 3.*population(uni_ind(nc-1),3,:);
		end
		if (nc>2)
			Third_Fit(1,trial,generation) = uni_fit(nc-2);
			Third_Pop(1,trial,generation) = population(uni_ind(nc-2),1,:);
			Third_Pop(1,trial,generation) += 2.*population(uni_ind(nc-2),2,:);
			Third_Pop(1,trial,generation) += 3.*population(uni_ind(nc-2),3,:);
		end


		First_Fit(1,trial,generation)
		First_Pop(1,trial,generation)
		Second_Fit(1,trial,generation)
		Second_Pop(1,trial,generation)
		Third_Fit(1,trial,generation)
		Third_Pop(1,trial,generation)
...
%			by
		...
		for iter=1:1:POPULATION_LIMIT
			for jter=1:1:3
				for kter=1:1:NUMBER_GENES
					if mutagen(MUTATION_RATE)
						if next_population(iter,jter,kter)>0
							next_population(iter,jter,kter)=0;
						else
							next_population(iter,jter,kter)=1;
						end
					end
					fixer(next_population(iter,jter,:),ADJ_MSK);
				end
			end
		end
		% End of mutation
